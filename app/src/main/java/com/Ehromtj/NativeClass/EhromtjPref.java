...
public class EhromtjPref {
    private static SharedPreferences sharedPreferences;
    private static EhromtjPref prefsInstance;
    ...
    public static void changeFeatureInt(String featureName, int featureNum, int value) {
        EhromtjPref.with(context).writeInt(featureNum, value);
    ...
    public static void changeFeatureString(String featureName, int featureNum, String str) {
        EhromtjPref.with(context).writeString(featureNum, str);
    ...
    public static boolean loadPrefBool(String featureName, int featureNum, boolean bDef) {
        boolean bool = EhromtjPref.with(context).readBoolean(featureNum, bDef);
        if (featureNum == -1) {
            loadPref = bool;
        }
		if (featureNum == -10)
    ...
    public static String loadPrefString(String featureName, int featureNum) {
        if (loadPref || featureNum <= 0) {
            String str = EhromtjPref.with(context).readString(featureNum);
            Changes(context, featureNum, featureName, 0, false, str);
            return str;
        }
        return "";
    ...
    private EhromtjPref(Context context) {
    ...
    private EhromtjPref(Context context, String preferencesName) {
        sharedPreferences = context.getApplicationContext().getSharedPreferences(
			preferencesName,
			Context.MODE_PRIVATE
        );
     ...
     */
    public static EhromtjPref with(Context context) {
        if (prefsInstance == null) {
            prefsInstance = new EhromtjPref(context);
        }
     ...
     * @return Returns a 'Preferences' instance
     */
    public static EhromtjPref with(Context context, boolean forceInstantiation) {
     ...
     * @return Returns a 'Preferences' instance
     */
    public static EhromtjPref with(Context context, String preferencesName) {
        if (prefsInstance == null) {
            prefsInstance = new EhromtjPref(context, preferencesName);
        }
        return prefsInstance;
    }
